//Primeiro exemplo testando IPERF emulado entre dois hosts
...
int main (int argc, char *argv[]) {
	...
	CommandLine cmd;
	cmd.AddValue ("banda", "Largura da banda padrão: 1m", bandWidth);
	cmd.Parse (argc, argv);
	...
	NetDeviceContainer dev, dev2;
	dev = p2p.Install(hosts);
	dev2 = p2p.Install(hosts);
	...
	Ipv4InterfaceContainer interfaces = enderecos.Assign (dev);

	//Fazendo o mesmo para o segundo nó
	interfaces = address.Assign (dev2);
	...
	programa = dceH.Install(hosts.Get(0));
	...
	programa = dceH.Install(hosts.Get(1));
